...
try {
  ...
  // ── 3b. sync UI: the "Связать с другим ПК" wizard opens and generates a code
  await client.send('WebDriver:Navigate', { url: `${base}/newtab.html` });
  const syncUi = await client.send('WebDriver:ExecuteAsyncScript', {
    script: `
      const done = arguments[arguments.length - 1];
      (async () => {
        const errs = [];
        window.addEventListener('error', (e) => errs.push(e.message));
        window.addEventListener('unhandledrejection', (e) => errs.push('rej: ' + e.reason?.message));
        let btn = null;
        for (let i = 0; i < 80; i++) {
          btn = document.getElementById('sync-link');
          if (btn) break;
          await new Promise((r) => setTimeout(r, 100));
        }
        ...
        for (const el of document.querySelectorAll('.modal-overlay')) {
          if (el.querySelector('#sync-code')) m = el;
        }
        const wizard = {
          toolbarBtn: !!btn,
          clickedId: clicked,
        ...
        // Type a code and continue — must generate the offer blob (WebRTC).
        const code = m?.querySelector('#sync-code');
        const next = m?.querySelector('#sync-next');
        if (code && next) { code.value = '7291'; next.click(); }
        for (let i = 0; i < 60; i++) {
          const out = document.querySelector('#sync-blob-out');
          if (out && out.value && out.value.length > 50) break;
          await new Promise((r) => setTimeout(r, 200));
        }
        const out = document.querySelector('#sync-blob-out');
        // Surface why the offer never appeared: a rejected promise would have
        // shown a notification toast instead.
        done({
          ...wizard,
          ...
          pageErrors: errs,
          hasRTCPeerConnection: typeof RTCPeerConnection,
  ...
  if (syncUi.value.toolbarBtn) pass('sync toolbar button present');
  else fail('sync toolbar button present', 'no #sync-link in DOM');
  if (syncUi.value.modalOpen && syncUi.value.title) pass('pair wizard opens', syncUi.value.title);
  else fail('pair wizard opens', JSON.stringify(syncUi.value));
  if (syncUi.value.offerBlobLen > 100) pass('offer code generated (WebRTC + compression)', `blobLen=${syncUi.value.offerBlobLen}`);
  else fail('offer code generated', `blobLen=${syncUi.value.offerBlobLen}`);

  // Options page: the linked-devices section renders.
  await client.send('WebDriver:Navigate', { url: `${base}/options/options.html` });
  const syncOpts = await client.send('WebDriver:ExecuteAsyncScript', {
    script: `
      const done = arguments[arguments.length - 1];
      (async () => {
        for (let i = 0; i < 60; i++) {
          if (document.querySelector('#syncPairings')) break;
          await new Promise((r) => setTimeout(r, 100));
        }
        done({
          section: !!document.querySelector('#syncPairings'),
          linkBtn: document.getElementById('syncLink')?.textContent,
          acceptBtn: document.getElementById('syncAccept')?.textContent,
          empty: document.querySelector('.sync-empty')?.textContent,
        });
      })();
    `,
    args: [],
  });
  if (syncOpts.value.section && syncOpts.value.linkBtn) pass('options sync section renders', JSON.stringify(syncOpts.value));
  else fail('options sync section renders', JSON.stringify(syncOpts.value));
